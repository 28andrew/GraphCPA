...
class GeneAutoencoder(nn.Module):
    def __init__(self, num_genes, intermediate_dim, latent_dim):
        ...
        # GCN layers
        self.conv1 = geom_nn.GCNConv(1, 1)
        ...
        self.edge_index = torch.load('../correlation_edge_index.pt').to(next(self.parameters()).device)
    ...
    def decode(self, z):
        ...
        # determine number of chunks to avoid memory runout
        num_chunks = 100  # set this according to your requirements

        edge_index_chunks = torch.chunk(self.edge_index, num_chunks, dim=1)

        num_edges = self.edge_index.shape[1]

        for embedding in embeddings:
            g_orig = self.decoder(embedding)
            g = g_orig.unsqueeze(1).to(device)

            g = self.conv1(g, self.edge_index)
            g = g.squeeze()
