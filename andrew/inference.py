...
from graphCPA import GeneAutoencoder, RDKitMoleculeEncoder, DrugPerturbationEncoder, CELL_TYPES


def predict(perturb_encoder, autoencoder, cell_type, smiles):
    # Convert inputs to tensor
    cell_type_tensor = torch.tensor([cell_type])  # If cell_type is an integer
    ...
    prediction = autoencoder.decode(emb)
    return prediction
...
if __name__ == '__main__':
    ...
    ckpt = torch.load('./checkpoints/ckpt_epoch_1000.pth', map_location=device)
    ...
    print('Done')
    df.to_csv('inference1000.csv', index=False)
